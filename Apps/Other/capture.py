...
for r in results:
    ...
    cv.imwrite('process/steps/01_rects.jpg',rects)
    
    croppedimg = image[y1:y2, x1:x2]
    cv.imwrite('process/steps/02_crop_rects.jpg', croppedimg)
    
    gray = cv.cvtColor(croppedimg, cv.COLOR_BGR2GRAY)
    if np.mean(gray) > 128:
             # white plate
         gray = 255 - gray
    cv.imwrite('process/steps/03_gray.jpg', gray)
    
    _,binary = cv.threshold(gray, 200, 255, cv.THRESH_BINARY+cv.THRESH_OTSU)
    cv.imwrite('process/steps/04_binary.jpg', binary)
    
    erode = cv.erode(binary, (3,3))
    cv.imwrite('process/steps/05_erode.jpg', erode)
    
    dilate = cv.erode(erode, (3,3))
    cv.imwrite('process/steps/06_dilate.jpg', dilate)
    ...
    for contour in contours:
        ...
        if h/croppedimg.shape[0]>=0.3 and 1<=ratio<=3.5:
            ...
            cv.imwrite('process/steps/07_contour.jpg',image_copy )
...
cv.imwrite('process/steps/08_final_result.jpg', image)

img = mpimg.imread('process/steps/08_final_result.jpg')
